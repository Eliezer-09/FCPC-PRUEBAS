...
export class LayoutComponent implements OnInit, AfterViewInit {
  ...
  getIdFuncionario() {
    this.idEntidad = this.localServiceS.getItem("id");
  }

  cargarTickets() {
    this.ticketService
      .getTicketsGenerados(this.idEntidad)
      .subscribe((res: any) => {
        this.ticketsGenerados = res["result"];
      });

    this.ticketService
      .getTicketsInternosByEstado("Pendiente", this.idEntidad)
      .subscribe((response) => {
        this.notificaciones = response["result"];
        this.countNotificaciones = response["result"].length;
      });

    this.ticketService
      .getTicketsSolicitudes(this.idEntidad)
      .subscribe((response) => {
        this.ticketsSolicitados = response["result"];
      });
  }
}
