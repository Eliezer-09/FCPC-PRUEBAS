...
export class LegalizarComponent implements OnInit, AfterViewChecked {
    ...
    this.creditoService
      ...
      .subscribe(
        ...
        (error) => {
          ...
          this.component.alerta(
            "error",
            "Ocurrió un error al descargar el documento: " + error
          );
        }
      );
  }
  ...
  getTablaInformativa() {
    ...
    document.body.appendChild(link);
    let url;
    if(this.prestamo.esNovacion)
    url="https://api.fcpc-cte.com/files/tablas/TABLA INFORMATIVA DE COSTOS - CRÉDITOS QUIROGRAFARIOS NOVACIÓN.pdf"
    if(this.prestamo.restructurado)
    url="https://api.fcpc-cte.com/files/tablas/TABLA INFORMATIVA DE COSTOS - CRÉDITOS REESTRUCTURADOS.pdf"
    if(this.prestamo.refinanciado)
    url="https://api.fcpc-cte.com/files/tablas/TABLA INFORMATIVA DE COSTOS - CRÉDITOS REFINANCIADOS.pdf"
    if(!this.prestamo.esNovacion && !this.prestamo.restructurado && !this.prestamo.refinanciado){
      if(this.prestamo.idProducto==1)   url="https://api.fcpc-cte.com/files/tablas/TABLA INFORMATIVA DE COSTOS - CRÉDITOS QUIROGRAFARIOS NUEVOS.pdf"
      if(this.prestamo.idProducto==3)   url="https://api.fcpc-cte.com/files/tablas/TABLA INFORMATIVA DE COSTOS - CRÉDITOS PRENDARIOS.pdf"
      if(this.prestamo.idProducto==9)   url="https://api.fcpc-cte.com/files/tablas/TABLA INFORMATIVA DE COSTOS - CRÉDITOS HIPOTECARIOS.pdf"
    }
    window.open(url);
    document.body.removeChild(link);
    this.spinner.hide();
  ...
  handleAdjunto(
    ...
    this.dataService.getBase64(this.fileToUpload).then(
      ...
      (error) => {
        this.component.alerta("error", "Ocurrió un error al traer el adjunto");
      }
    );
  }
  obtenerNovacion() {
