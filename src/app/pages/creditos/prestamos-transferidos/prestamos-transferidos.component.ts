...
@UntilDestroy()
@Component({
  selector: 'vex-prestamos-transferidos',
  templateUrl: './prestamos-transferidos.component.html',
  styleUrls: ['./prestamos-transferidos.component.scss'],
...
export class PrestamosTransferidosComponent implements OnInit {
  pageSize = 10;
  isLoading = true;
  infoMessage = "No se ha encontrado créditos tránsferidos";
  term = '';
  status="Transferido";
  ...
  prestamosTransferidos: OperationResultPrestamo = {};
  pageEvent: PageEvent;
  filterValue: string = null;
  routers :any[]=[];

  icroundSearch           = iconify.icroundSearch;
  icroundCardMembership    = iconify.icroundCardMembership;
...
/*    {
  ...
  generateRouter(event){
    ...
    this.routers.push('completado')
  }

  servicesPrestamo(status,id,page,pageSize){
    this.creditoService.getPrestamosByEstado(status, id,page, pageSize).pipe(
      map((prestamosTransferidos:OperationResultParticipe) => {
        this.isLoading = false
        this.prestamosTransferidos = prestamosTransferidos;
        this.dataFondoSource.data = prestamosTransferidos.result;
