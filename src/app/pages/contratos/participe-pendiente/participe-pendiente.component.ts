...
@UntilDestroy()
@Component({
  selector: 'vex-participe-pendiente',
  templateUrl: './participe-pendiente.component.html',
  styleUrls: ['./participe-pendiente.component.scss'],
...
export class ParticipePendienteComponent implements OnInit {
  pageSize = 10;
  isLoading = true;
  infoMessage = "No se ha encontrado participes pendientes";
  term = '';
  status="Pendiente";
  ...
  pendientes: OperationResultParticipe = {};
  pageEvent: PageEvent;
  filterValue: string = null;
  routers :any[]=[];

  icroundSearch           = iconify.icroundSearch;
  icroundPersonSearch     = iconify.icroundPersonSearch;
  icroundManageSearch     = iconify.icroundManageSearch;
 ...
 /*    {
  ...
  constructor(    
    private data: DataService, private contratoService: ContratosService, 
  ...
  servicesParticipe(status,page,pageSize){
    this.contratoService.getParticipeByEstado(status, page, pageSize).pipe(
      map((pendientes:OperationResultParticipe) => {
        this.isLoading = false
        this.pendientes = pendientes;
        this.dataFondoSource.data = pendientes.result;
