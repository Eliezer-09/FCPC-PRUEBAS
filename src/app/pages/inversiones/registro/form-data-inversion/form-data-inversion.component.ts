...
export class FormDataInversionComponent implements OnInit, AfterViewChecked {
  ...
  changeEmisionDate(){
    ...
    this.changeLimitVencimientoDate(fechaEmision._d);
  ...
  calculeDaysEmisionVensiomiento(fechaEmision,fechaVencimiento) {
    if(fechaVencimiento != ''){
      if (fechaEmision > fechaVencimiento) {
        ...
        this.formGenerales.controls['totalPlazo'].setValue(numero);
  ...
  calculateCostoCompra(){
    const casaValores=parseFloat(this.formGenerales.value.costoCasaValores);
    const costoBolsaValores=parseFloat(this.formGenerales.value.costoBolsaValores);
    const impuestoValue=parseFloat(this.formGenerales.value.impuestos);
  ...
  enableDesmaterializado(event) {
    if (event["checked"]  ||  event==true) {
      ...
      if(this.formGenerales.value.idTipoInversion!=TiposTituloInversiones.AccionesNoCotizadas){
  ...
  calculateValorNominal(){
    ...
    if(nAcciones && precioAccion){
      ...
      this.formGenerales.controls['valorNominal'].setValue(valorNominal);
  ...
  calculateNumeroAcciones(){
    ...
    if(ndesde && nhasta){
      ...
      this.formGenerales.controls['numeroAcciones'].setValue(nAciones);
  ...
  calculoAcciones(){
    ...
    if (hastaAccion > desdeAccion) {
      this.formGenerales.get('numeroAcciones').setValue(hastaAccion  - desdeAccion);
    }
  }
  ...
  ValidationDiasPlazo(){
    ...
    if( plazoDias <  periodoGracia){
      this.formGenerales.controls['plazoGracia'].setValue("")
  ...
  verifyInputNumber(){
    ...
    if(this.formGenerales.value.idTipoInversion==TiposTituloInversiones.AccionesNoCotizadas)  controlsNumber=["valorEnLibro","valorNominal","valorCompra","valorMercado","tasa","precioPorAccion"]
  ...
  seleccionarEmisor(Emisor) {
    if (Emisor.idEntidad) {
      ...
      if(this.formGenerales.value.idTipoInversion!=TiposTituloInversiones.AccionesNoCotizadas) this.changeDataEmisor(Emisor.idEntidad);
    }
  }
