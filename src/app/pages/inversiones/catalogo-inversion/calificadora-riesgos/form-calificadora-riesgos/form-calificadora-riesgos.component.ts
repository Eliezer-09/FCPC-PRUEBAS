...
@Component({
  selector: "vex-form-calificadora-riesgos",
  templateUrl: "./form-calificadora-riesgos.component.html",
  styleUrls: ["./form-calificadora-riesgos.component.scss"],
  animations: [
    stagger80ms,
    fadeInUp400ms,
    scaleIn400ms,
...
export class FormCalificadoraRiesgosComponent implements OnInit, AfterViewChecked {
  ...
  constructor(
    ...
    loadPaises() {
      this.dataService.getPaises().subscribe((res: any) => {
        this.paises = res;
      });
    }
    ...
    getIDType(){
      this.dataService.getTiposIdentificaciones().subscribe( (tiposIdentificaciones:any) => {
        this.tiposIdentificaciones = tiposIdentificaciones.filter(
          (IDType) => IDType.codigoSbs == "R" || IDType.codigoSbs == "IE"
        );
      });
    }
