...
@Component({
  selector: 'vex-reportes-anexo',
  templateUrl: './reportes-anexo.component.html',
  styleUrls: ['./reportes-anexo.component.scss'],
  animations: [fadeInUp400ms, stagger80ms],
})
export class ReportesAnexoComponent implements OnInit {
  isLoading = true;
  isCard = true;
  pageSize = 12;
  infoMessage = "No se ha encontrado anexos";
  term = "";
  ...
  loadReportsByTerm(page, pageSize) {
    this.reportesService
      .getAnexoReports(page, pageSize, this.term)
      .pipe(
        map((reports: any) => {
          this.isLoading = false;
          this.reportsresult = reports;
  ...
  generateReport(data,actualizar) {
    ...
    let formData = data["formData"];
    this.reportesService
      .getAnexosFile(codigo, format, formData,actualizar)
      .subscribe(
        (response) => {
        ...
        (error) => {
          ...
          this.componentesService.alerta(
            "error",
            "Ocurrio un error al intentar generar el reporte."
          );
          this.spinner.hide();
        }
      );
